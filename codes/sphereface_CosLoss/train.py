...
parser.add_argument('--dataset', default='../data/train/A', type=str)
...
def train(epoch,args):
    ...
    total = 0
    batch_idx = 0
    ds = ImageDataset(args.dataset,dataset_load,'../data/train/APDlandmark.txt',name=args.net+':train',
    ...
    while True:
        ...
        outputs = outputs # 0=cos_theta 1=phi_theta
...
for epoch in range(0, 120):
    ...
    # modifications              #
    ##############################
    if epoch in [0,10,30,60]:
        if epoch!=0: args.lr *= 0.1
        optimizer = optim.SGD(net.parameters(), lr=args.lr, momentum=0.9, weight_decay=5e-4)

    train(epoch,args)
    if epoch in range(0, 120, 3):
        save_model(net, './models/{}_{}.pth'.format(args.net,epoch))
